...
def candlesticks(ts,ax,volume=False,vol_ax=None,style="candlesticks"):
    ...
    if style=="candlesticks":
        if volume is False:
            ...
            mplf.plot(ts.df,ax=ax,volume=vol_ax,type='candle',style='yahoo',show_nontrading=True)
    elif style=="ohlc":
        mplf.plot(ts.df,
            ax=ax,
            type="ohlc",
...
def plot_moving_averages(ts,ax,windows=None):
    ...
    if windows is None:
        windows=[
            ts.ta_params["sma"]["fast"],
            ts.ta_params["sma"]["slow"]
        ]
    for window,color in zip(windows,colors):
        ax.plot(ts.df["SMA_"+str(window)],linestyle="-",color=color,label="SMA"+str(window),alpha=0.3,linewidth=1)
...
def plot_bb(ts,iax):
    ...
    iax.plot(ts.df['BBM_'+length+"_"+std], label=f'MA{length}-STD{str(std)}',color='dodgerblue', alpha=0.35) #middle band
    iax.plot(ts.df['BBU_'+length+"_"+std], label='_', color='greenyellow', alpha=0.35) #Upper band
    iax.plot(ts.df['BBL_'+length+"_"+std], label='_', color='coral', alpha=0.35) #lower band
    iax.fill_between(ts.df.index, ts.df['BBL_'+length+"_"+std], ts.df['BBU_'+length+"_"+std], alpha=0.1)
